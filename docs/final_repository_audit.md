...
- Open final-parity access blockers and remaining validation/implementation gates remain documented with owners and evidence expectations.
...
| Pre_Regulation_Ext | Yes | Yes (`03_*`, `06_*`) | Yes (selected-source certification and non-price certifications) |
| Regulation movements | Yes | Yes (`03_regulation_movments_validation.sql`) | Yes (migration/split-price dependencies) |
| Hedge liquidity / SCD | Yes | Yes (`04_hedge_liquidity_validation.sql`) | Yes (hedge source validation and SCD seed policy) |
...
| MIFID2_Hedge_Report | Yes | Yes (`08_*_validation.sql`) | Yes (SCD; RecordID; transaction-reference parity) |
...
### Reclassified former blockers (not active blocker category)

| Object / topic | Current status |
| --- | --- |
| `main.trading.bronze_etoro_trade_currencyprice` | Readable but not preferred; primary `Reg_CurrencyPrice_Ext` source selected elsewhere |
...
| Topic | Status |
| --- | --- |
| `CurrencyPriceMaxDateWithSplit` selected source validation | Selected (`main.dealing.bronze_pricelog_candles_currencypricemaxdatewithsplit`); baseline/date-window validation pending (MAG-14; D-05) |
| Exact CFI / `InstrumentClassification` parity | Hard parity requirement; SQL Server value matching validation pending (MAG-15; D-14) |
| Hedge `RecordID` strategy | Direction approved (historical preserve + max+1 registry); implementation/validation pending (MAG-12; D-12) |
| Hedge `TransactionReferenceNumber` parity | Hard parity requirement; SQL Server value matching validation pending (MAG-13; D-13) |
...
| `main.dealing.bronze_pricelog_history_currencyprice` | Selected primary source for `Reg_CurrencyPrice_Ext` (`History.CurrencyPrice` / `History.CurrencyPrice_Active`) |
| `main.dealing.bronze_pricelog_candles_currencypricemaxdatewithsplit` | Selected primary source for `Reg_Ext_CurrencyPriceMaxDateWithSplit` |
| `main.trading.bronze_etoro_trade_currencyprice` | Reclassified to readable but not preferred (fallback/reference) |
| `main.bi_db.bronze_etoro_hedge_hedgeservertoliquidityaccount` | Reclassified to readable; required columns present |
...
| Recommended next phase | **Blocker resolution and controlled execution planning** (close active access blockers and remaining validation/seed/parity gates, record MAG approvals externally, then enable SELECT-only validation and gated staging per `docs/final_validation_execution_plan.md`) |
...
Step 18B handoff artifacts are now available: `docs/final_handoff_package.md` and `docs/final_manager_handoff_summary.md`.
