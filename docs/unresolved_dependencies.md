...
- Historical seed strategy direction is approved (seed required history; if minimum safe window is unproven, seed all available history), with seed implementation and extract ownership still pending.
...
| Step 9 `MIFID2_Failed_TRAX` history/current dependency | Step 9 requires latest-row logic over `MIFID2_NPD_TRAX`; policy direction is approved, but implementation-window ownership and source availability are pending | Failed-TRAX customer supplementation can be incomplete or non-deterministic for requested validation windows | Implement approved seed policy, assign extract/window ownership, and confirm `main.regtech_ops_stg.bi_output_regtechops_mifid2_npd_trax` availability before un-gating `06_failed_trax_staging.sql` | Yes |
...
20. Complete Step 7 liquidity-source required-column, duplicate/key, and coverage certification for already accessible sources.
21. Implement approved Step 7 SCD seed/cutover strategy and finalize removed-account `IsLast` parity/correction policy.
...
- Liquidity SCD seed/cutover gate:
  - `main.regtech_ops_stg.bi_output_regtechops_reg_liquidtyacount_scd` seed/cutover direction is approved (D-09 / MAG-11); runbook-level implementation, extract ownership, and validation remain pending before activation.
- LEI coverage gate:
...
- History/seed direction:
  - strategy approved to seed required history (or all available if minimum safe window is unproven); implementation and extract ownership remain pending
